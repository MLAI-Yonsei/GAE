#!/bin/bash
# Train + evaluate SAEBoost residual for Adversarial OOD.
...
OOD_SET=${3:-halu_eval}            # halu_eval jailbreakhub
SAEBOOST_COEF=${4:-1.0}
RESIDUAL_CKPT_ARG=${5:-}
...
if [ -n "$RESIDUAL_CKPT_ARG" ]; then
    ...
    RESIDUAL_CHECKPOINT="${REPO_DATA}/checkpoints/adversarial_ood/${MODEL}_transcoder_${OOD_SET}_saeboost_resid_${COEF_SUFFIX}.pt"
fi
