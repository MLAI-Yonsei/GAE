...
class RunnerConfig(ABC):
    ...
    dataset_text_field: Optional[Any] = None
    dataset_override: Optional[Any] = None
    dataset_streaming: bool = False  # False = download to local cache (stable), True = stream from HF (may hang)
    dataset_split: str = "train"  # dataset split to use (e.g. "train", "train[:5%]")
    datasets_cache_dir: Optional[str] = None  # HF datasets cache directory
...
class LanguageModelSAERunnerConfig(RunnerConfig):
    ...
    # Activation function: "relu" (vanilla L1 SAE) or "topk" (TopK SAE, Gao et al. 2024)
    activation_fn: str = "relu"
    topk_k: Optional[int] = None  # required when activation_fn="topk"
    topk_aux_coeff: float = 1.0 / 32  # auxiliary dead-feature loss weight (OpenAI default)
    ...
    wandb_histogram_max_points: int = 5000
    ...
    def __post_init__(self):
        ...
        if self.activation_fn == "topk":
            self.run_name = f"{self.d_sae}-TopK-{self.topk_k}-LR-{self.lr}-Tokens-{self.total_training_tokens:3.3e}"
        else:
        ...
        if self.objective_type not in ["ERM", "TERM"]:
            raise ValueError(
                f"objective_type must be ERM or TERM. Got {self.objective_type}"
            )

        if self.activation_fn not in ["relu", "topk"]:
            raise ValueError(
                f"activation_fn must be relu or topk. Got {self.activation_fn}"
            )
        if self.activation_fn == "topk":
            if self.topk_k is None:
                raise ValueError("topk_k must be set when activation_fn='topk'")
            if self.topk_k <= 0:
                raise ValueError(f"topk_k must be positive. Got {self.topk_k}")
            if self.topk_k > self.d_sae:
                raise ValueError(f"topk_k ({self.topk_k}) must be <= d_sae ({self.d_sae})")
