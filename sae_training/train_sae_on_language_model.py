...
def train_sae_on_language_model(
    ...
    if getattr(cfg, "reinit_b_dec", True):
        sparse_autoencoder.initialize_b_dec(activation_store)
    else:
        print("Skipping b_dec reinitialization (reinit_b_dec=False)")
    ...
    while n_training_tokens < total_training_tokens:
        ...
        is_topk_sae = sparse_autoencoder.activation_fn == "topk"
        if (not is_topk_sae) and (feature_sampling_method=="anthropic") and ((n_training_steps + 1) % dead_feature_window == 0):
            ...
            if len(dead_neuron_indices) > 0:
                ...
                if len(dead_neuron_indices) > cfg.resample_batches * cfg.store_batch_size:
        ...
        if (not is_topk_sae) and (feature_sampling_method == "l2") and ((n_training_steps + 1) % dead_feature_window == 0):
        ...
        if (n_training_steps + 1) % feature_sampling_window == 0:
            ...
            if use_wandb:
                max_hist_points = getattr(cfg, "wandb_histogram_max_points", 5000)
                if log_feature_sparsity.numel() > max_hist_points:
                    idx = torch.linspace(
                        0,
                        log_feature_sparsity.numel() - 1,
                        steps=max_hist_points,
                    ).long()
                    histogram_values = log_feature_sparsity[idx]
                else:
                    histogram_values = log_feature_sparsity
                wandb_histogram = wandb.Histogram(histogram_values.numpy())
                wandb.log(
                    {   
        ...
        with torch.no_grad():
            ...
            if use_wandb and ((n_training_steps + 1) % wandb_log_frequency == 0):
                ...
                is_topk = sparse_autoencoder.activation_fn == "topk"
                log_dict = {
                    # losses
                ...
                if is_topk:
                    log_dict["losses/aux_loss"] = ghost_grad_loss.item()
                else:
                    log_dict["losses/l1_loss"] = l1_loss.item() / sparse_autoencoder.l1_coefficient
                    log_dict["losses/ghost_grad_loss"] = ghost_grad_loss.item()
                wandb.log(log_dict, step=n_training_steps)
            ...
            if sparse_autoencoder.cfg.use_tqdm:
                ...
                elif sparse_autoencoder.activation_fn == "topk":
                    pbar.set_description(
                        f"{n_training_steps}| MSE Loss {mse_loss.item():.3f} | Aux {ghost_grad_loss.item():.3f}"
                    )
                else:
                    pbar.set_description(
        ...
        if n_checkpoints > 0 and n_training_tokens > checkpoint_thresholds[0]:
            ...
            if use_wandb:
                try:
                    ...
                    print(f"Warning: wandb artifact upload failed: {e}. Continuing training.")
    ...
    sparse_autoencoder.save_model(path)
    if "log_feature_sparsity" not in locals():
        feature_sparsity = act_freq_scores / n_frac_active_tokens
        log_feature_sparsity = torch.log10(feature_sparsity + 1e-10).detach().cpu()
    torch.save(log_feature_sparsity, log_feature_sparsity_path)
    if use_wandb:
        try:
            cfg = sparse_autoencoder.cfg
            # Sanitize cfg metadata: strip non-JSON-serializable fields
            # (e.g. split-aware text_field closures from adversarial OOD).
            metadata = dict(cfg.__dict__)
            for _k in ("dataset_text_field", "dataset_override"):
                _v = metadata.get(_k)
                if callable(_v) or (
                    _v is not None
                    and not isinstance(_v, (str, int, float, bool, list, tuple, dict, type(None)))
                ):
                    metadata[_k] = None
            sparsity_artifact = wandb.Artifact(
                    f"{sparse_autoencoder.get_name()}_log_feature_sparsity", type="log_feature_sparsity", metadata=metadata
                )
            sparsity_artifact.add_file(log_feature_sparsity_path)
            wandb.log_artifact(sparsity_artifact)
        except Exception as e:
            print(f"Warning: final wandb artifact upload failed: {e}. Continuing.")
