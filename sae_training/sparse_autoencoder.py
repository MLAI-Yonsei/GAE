...
class SparseAutoencoder(HookedRootModule):
    ...
    def __init__(
        ...
        self.activation_fn = getattr(cfg, "activation_fn", "relu")
        self.topk_k = getattr(cfg, "topk_k", None)
        self.topk_aux_coeff = getattr(cfg, "topk_aux_coeff", 1.0 / 32)
    ...
    def _apply_activation(self, hidden_pre):
        """Apply the configured activation function to encoder pre-activations."""
        if self.activation_fn == "topk":
            # TopK: keep top-k values, zero the rest, then ReLU for non-negativity
    ...
    def _compute_topk_aux_loss(self, hidden_pre, sae_out, mse_target, dead_neuron_mask):
        """
        Auxiliary loss for TopK SAE to reduce dead features (Gao et al. 2024).
        Reconstructs the residual error using only dead features with top-k_aux selection.
        """
        if dead_neuron_mask is None or dead_neuron_mask.sum() == 0:
            return torch.tensor(0.0, dtype=self.dtype, device=self.device)

        # Detach residual so aux loss gradients only flow through dead features
        residual = (mse_target.float() - sae_out.float()).detach()

        # Activate dead features only with top-k_aux selection
        dead_pre = hidden_pre[:, dead_neuron_mask]
        k_aux = min(self.topk_k, dead_pre.shape[-1])
        if k_aux == 0:
            return torch.tensor(0.0, dtype=self.dtype, device=self.device)

        topk_values, topk_indices = torch.topk(dead_pre, k_aux, dim=-1)
        dead_acts = torch.zeros_like(dead_pre)
        dead_acts.scatter_(-1, topk_indices, F.relu(topk_values))

        # Reconstruct residual from dead features
        dead_recon = dead_acts @ self.W_dec[dead_neuron_mask, :]

        # Normalized MSE of dead-feature reconstruction vs residual
        residual_norm_sq = residual.pow(2).sum(dim=-1, keepdim=True).clamp(min=1e-8)
        aux_loss = (dead_recon - residual).pow(2).sum(dim=-1) / residual_norm_sq.squeeze(-1)
        return self.topk_aux_coeff * aux_loss.mean()
    ...
    def forward(self, x, dead_neuron_mask = None, mse_target=None):
        ...
        feature_acts = self.hook_hidden_post(self._apply_activation(hidden_pre))
        ...
        # Per-sample normalized MSE: ||sae_out - target||^2 / ||target||^2
        if mse_target is None:
            per_sample_mse = torch.pow((sae_out-x.float()), 2).sum(dim=-1) / (x**2).sum(dim=-1).clamp(min=1e-8)
        else:
            per_sample_mse = torch.pow((sae_out-mse_target.float()), 2).sum(dim=-1) / (mse_target**2).sum(dim=-1).clamp(min=1e-8)

        # MSE loss with objective (computed before ghost grads which reference it)
        if self.cfg.objective_type == "ERM":
            mse_loss = erm_loss(per_sample_mse)
        elif self.cfg.objective_type == "TERM":
            mse_loss = term_loss(per_sample_mse, self.cfg.term_t)
        else:
            raise ValueError(f"Unknown objective_type: {self.cfg.objective_type}")

        # Auxiliary / ghost gradient loss
        mse_loss_ghost_resid = torch.tensor(0.0, dtype=self.dtype, device=self.device)

        if self.activation_fn == "topk":
            # TopK: use auxiliary dead-feature loss instead of ghost gradients
            if self.training and dead_neuron_mask is not None and dead_neuron_mask.sum() > 0:
                effective_target = mse_target if mse_target is not None else x
                mse_loss_ghost_resid = self._compute_topk_aux_loss(
                    hidden_pre, sae_out, effective_target, dead_neuron_mask
                )
        else:
            # ReLU: use ghost gradients if enabled
        ...
        # Sparsity loss: L1 for ReLU, zero for TopK (sparsity is structural)
        if self.activation_fn == "topk":
    ...
    def set_decoder_norm_to_unit_norm(self):
        if self.activation_fn == "topk":
            return  # TopK SAE: decoder norms are unconstrained (Gao et al. 2024)
        self.W_dec.data /= torch.norm(self.W_dec.data, dim=1, keepdim=True)
    ...
    def remove_gradient_parallel_to_decoder_directions(self):
        ...
        '''
        if self.activation_fn == "topk":
            return  # TopK SAE: no decoder norm constraint, so no gradient projection needed
    ...
    def save_model(self, path: str):
        ...
        # Sanitize cfg before pickling: some training paths install callables
        # (e.g. split-aware text_field closures) or live dataset objects into
        # cfg, which break torch.save. Build a shallow copy that strips fields
        # we can't pickle safely.
        try:
            import copy as _copy_mod
            saved_cfg = _copy_mod.copy(self.cfg)
        except Exception:
            saved_cfg = self.cfg
        for _attr in ("dataset_text_field", "dataset_override"):
            if hasattr(saved_cfg, _attr):
                _val = getattr(saved_cfg, _attr)
                if callable(_val) or (_val is not None and not isinstance(_val, (str, int, float, bool, list, tuple, dict, type(None)))):
                    try:
                        setattr(saved_cfg, _attr, None)
                    except Exception:
                        pass
    ...
    def get_name(self):
        if self.activation_fn == "topk":
            sae_name = f"sparse_autoencoder_{self.cfg.model_name}_{self.cfg.hook_point}_{self.cfg.d_sae}_topk{self.topk_k}"
        else:
