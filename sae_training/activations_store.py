...
class ActivationsStore:
    ...
    @staticmethod
    def _try_load_from_arrow_cache(dataset_path, cache_dir):
        """Try to load dataset from pre-cached arrow files in the HF cache directory."""
        # Map common dataset paths to their cache directory names
        ds_name = dataset_path.replace("/", "___").lower()
        # Search for arrow files matching this dataset
        for root, dirs, files in os.walk(cache_dir):
            # Check if dataset name appears in the path
            if ds_name not in root.lower() and dataset_path.split("/")[-1].lower() not in root.lower():
                continue
            arrow_files = sorted(glob.glob(os.path.join(root, "*.arrow")))
            if len(arrow_files) >= 1:
                print(f"Found {len(arrow_files)} arrow files in {root}")
    ...
    def __init__(
        ...
        self.cfg = cfg
        self.model = model
        if getattr(cfg, "dataset_override", None) is not None:
            self.dataset = cfg.dataset_override
        else:
            streaming = getattr(cfg, "dataset_streaming", False)
            split = getattr(cfg, "dataset_split", "train")
            cache_dir = getattr(cfg, "datasets_cache_dir", None)

            # Try to load from pre-cached arrow files first (avoids HF re-download)
            loaded_from_arrow = False
            if not streaming and cache_dir:
                arrow_dataset = self._try_load_from_arrow_cache(cfg.dataset_path, cache_dir)
                if arrow_dataset is not None:
                    self.dataset = arrow_dataset
                    loaded_from_arrow = True
                    print(f"Loaded dataset from local arrow cache ({len(arrow_dataset)} examples)")

            if not loaded_from_arrow:
                load_kwargs = dict(split=split, streaming=streaming)
                if cache_dir:
                    load_kwargs["cache_dir"] = cache_dir
                # Local file path: load as json dataset
                if os.path.exists(cfg.dataset_path):
                    print(f"Loading dataset from local file: {cfg.dataset_path}")
                    self.dataset = load_dataset("json", data_files=cfg.dataset_path, **load_kwargs)
                elif getattr(cfg, "dataset_name", None):
                    self.dataset = load_dataset(cfg.dataset_path, name=cfg.dataset_name, **load_kwargs)
                else:
                    self.dataset = load_dataset(cfg.dataset_path, **load_kwargs)
        self.iterable_dataset = iter(self.dataset)

        # helper for safely cycling finite datasets
        def _next_example():
            try:
                return next(self.iterable_dataset)
            except StopIteration:
                # restart iterator for finite datasets
                self.iterable_dataset = iter(self.dataset)
                try:
                    return next(self.iterable_dataset)
                except StopIteration:
                    raise StopIteration("Dataset appears to be empty.")

        self._next_example = _next_example
        
        # check if it's tokenized
        first = self._next_example()
        if "tokens" in first.keys():
    ...
    def get_batch_tokens(self):
        ...
        missing_field_streak = 0
        while batch_tokens.shape[0] < batch_size:
            if not self.cfg.is_dataset_tokenized:
                example = self._next_example()
                text_field = getattr(self.cfg, "dataset_text_field", None) or "text"
                s = _extract_text(example, text_field)
                if s is None:
                    # Adversarial datasets use callable text_fields that can
                    # legitimately return None for some examples (e.g. jailbreak
                    # rows without a valid split-specific text). Skip and move
                    # on; only raise after many consecutive misses so we still
                    # catch genuine schema mismatches.
                    missing_field_streak += 1
                    if missing_field_streak > 1000:
                        raise KeyError(
                            f"Dataset example is missing text field '{text_field}' "
                            f"after {missing_field_streak} consecutive examples. "
                            "Set cfg.dataset_text_field to the correct key."
                        )
                    continue
                missing_field_streak = 0
